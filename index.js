...
const app = createApp({
        ...
        updateProduct(){
           ...
           axios[http](updateProductUrl,{ data: this.tempProduct })
            .then((res)=>{
                ...
                myModal.hide(); // 套用 modal.hide() 方法關閉 model
    ...
    mounted() {
            myModal = new bootstrap.Modal(document.getElementById('showModal'),{
                // 禁止使用者使用 ESC 鍵關閉互動視窗
                keyboard: false,
                // 禁止使用者點擊 modal 以外的地方來關閉視窗，避免資料輸入到一半遺失
                backdrop: 'static'
            });
            deleteModel = new bootstrap.Modal(document.getElementById('delModel'),{
                 keyboard: false,
                 backdrop: 'static'
            });
